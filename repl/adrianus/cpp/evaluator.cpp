...
Ad_Object* Evaluator::EvalInfixExpression(std::string _operator, Ad_Object* left, Ad_Object* right) {
    ...
    if (left->Type() == OBJ_HASH && right->Type() == OBJ_HASH) {
        return EvalHashInfixExpression(_operator, left, right);
    }
...
Ad_Object* Evaluator::EvalHashInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) {
    std::unordered_map<std::string, HashPair> left_pairs = ((Ad_Hash_Object*)left)->pairs;
    std::unordered_map<std::string, HashPair> right_pairs = ((Ad_Hash_Object*)right)->pairs;
    if (_operator == "==") {
        // TODO: generated by cursor, it uses recursive calls to EvalInfixExpression, which is not efficient
        if (left_pairs.size() != right_pairs.size()) {
            return &FALSE;
        }
        for (std::unordered_map<std::string, HashPair>::iterator it = left_pairs.begin(); it != left_pairs.end(); ++it) {
            std::unordered_map<std::string, HashPair>::iterator right_it = right_pairs.find(it->first);
            if (right_it == right_pairs.end()) {
                return &FALSE;
            }
            Ad_Object* keys_equal = EvalInfixExpression("==", it->second.key, right_it->second.key);
            if (!IsTruthy(keys_equal)) {
                return &FALSE;
            }
            Ad_Object* values_equal = EvalInfixExpression("==", it->second.value, right_it->second.value);
            if (!IsTruthy(values_equal)) {
                return &FALSE;
            }
        }
        return &TRUE;
    ...
    if (_operator == "!=") {
        Ad_Object* equals = EvalHashInfixExpression("==", left, right);
        if (equals == nullptr) {
            return nullptr;
        }
        return NativeBoolToBooleanObject(equals == &FALSE);
    }
    return nullptr;
}
