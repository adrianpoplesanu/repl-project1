...
void thread_async_run(Ad_Object* rawObject, GarbageCollector *gc, Environment &env) {
    Ad_Thread_Object* threadObject = (Ad_Thread_Object*) rawObject;
    TOTAL_THREADS_RUNNING++;
    std::thread *th1 = new std::thread(ad_worker_async, threadObject->callback, threadObject->params, rawObject, threadObject->internal_gc, &env);
...
void thread_blocking_run(Ad_Object* rawObject, GarbageCollector *gc, Environment &env) {
    ...
    TOTAL_THREADS_RUNNING++;
    std::thread *th1 = new std::thread(ad_worker_blocking, threadObject->callback, threadObject->params, rawObject, threadObject->internal_gc, &env);
    threadObject->internal_thread = th1;
    threadPool.push_back(threadObject);
    th1->join();
}

void thread_await(Ad_Object* rawObject, GarbageCollector *gc, Environment &env) {
    Ad_Thread_Object* threadObject = (Ad_Thread_Object*) rawObject;
    if (threadObject->internal_thread->joinable()) {
        threadObject->internal_thread->join();
    }
}
