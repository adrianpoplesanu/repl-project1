...
Ad_Object* Evaluator::Eval(Ad_AST_Node* node, Environment &env) {
    switch(node->type) {
        ...
        case ST_RETURN_STATEMENT: {
            ...
            auto* obj = new Ad_ReturnValue_Object();
        ...
        break;
        case ST_INTEGER: {
            auto* obj = new Ad_Integer_Object();
            garbageCollector->addObject(obj);
        ...
        case ST_FUNCTION_LITERAL: {
            auto* obj = new Ad_Function_Object(((Ad_AST_FunctionLiteral*)node)->parameters, ((Ad_AST_FunctionLiteral*)node)->body, &env);
            garbageCollector->addObject(obj);
            // TODO: add obj to gc
            return obj;
        }
        ...
        case ST_LIST_LITERAL: {
            ...
            auto* obj = new Ad_List_Object(elements);
...
Ad_Object* Evaluator::EvalProgram(Ad_AST_Node* node, Environment &env) {
    ...
    for (auto* it : ((Ad_AST_Program*)node)->statements) {
        result = nullptr;
        Ad_AST_Node *obj = it;
        result = Eval(obj, env);
        if (result != nullptr) {
        ...
        if (result != nullptr && result->Type() == OBJ_SIGNAL) return result; // exit() builtin was used in order to trigger the stopping of the process
        if (result != nullptr && result->Type() == OBJ_ERROR) {
            // TODO: mark and sweep cleanup
...
void Evaluator::GarbageCollectEnvironments() const {
    //std::cout << "cleaning up environments\n";
    garbageCollector->sweepEnvironments();
}

Ad_Object* Evaluator::EvalInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) {
    if (left == nullptr || right == nullptr) {
        return nullptr;
    }
    if (left->Type() == OBJ_ERROR) {
        auto* error_obj = (Ad_Error_Object*) left;
        auto *obj = new Ad_Error_Object(error_obj->message);
        garbageCollector->addObject(obj);
        return obj;
    }
    if (right->Type() == OBJ_ERROR) {
        auto* error_obj = (Ad_Error_Object*) right;
        auto *obj = new Ad_Error_Object(error_obj->message);
        garbageCollector->addObject(obj);
        return obj;
    }
...
Ad_Object* Evaluator::EvalIntegerInfixExpression(std::string _operator, Ad_Object* left, Ad_Object* right) {
    ...
    if (_operator == "+") {
        auto* obj = new Ad_Integer_Object(left_val + right_val);
        garbageCollector->addObject(obj);
        return obj;
    }
    if (_operator == "-") {
...
Ad_Object* Evaluator::EvalBooleanInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) {
...
Ad_Object* Evaluator::EvalStringInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) {
    std::string left_val = ((Ad_String_Object*)left)->value;
    std::string right_val = ((Ad_String_Object*)right)->value;
    if (_operator == "+") {
        auto* obj = new Ad_String_Object(left_val + right_val);
        garbageCollector->addObject(obj);
        return obj;
    }
    if (_operator == "==") {
...
Ad_Object* Evaluator::EvalStringAndIntInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) const {
    std::string left_val = ((Ad_String_Object*)left)->value;
    int right_val = ((Ad_Integer_Object*)right)->value;
    if (_operator == "+") {
        auto* obj = new Ad_String_Object(left_val +  std::to_string(right_val));
        garbageCollector->addObject(obj);
        return obj;
    }
    return nullptr;
...
Ad_Object* Evaluator::EvalIntAndStringInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) const {
    int left_val = ((Ad_Integer_Object*)left)->value;
    std::string right_val = ((Ad_String_Object*)right)->value;
    if (_operator == "+") {
        auto* obj = new Ad_String_Object(std::to_string(left_val) +  right_val);
        garbageCollector->addObject(obj);
        return obj;
    }
    return nullptr;
...
Ad_Object* Evaluator::EvalFloatInfixExpression(const std::string& _operator, Ad_Object* left, Ad_Object* right) {
    float left_val = ((Ad_Float_Object*)left)->value;
    float right_val = ((Ad_Float_Object*)right)->value;
    if (_operator == "+") {
        auto* obj = new Ad_Float_Object(left_val + right_val);
        garbageCollector->addObject(obj);
        return obj;
    }
    if (_operator == "-") {
...
Ad_Object* Evaluator::EvalPrefixExpression(const std::string& _operator, Ad_Object* right) {
    if (_operator == "!") {
...
Ad_Object* Evaluator::EvalBangOperatorExpression(Ad_Object* right) {
    ...
    if (((Ad_Boolean_Object *) right)->value) {
        return &FALSE;
    }
    return &TRUE;
    return &FALSE;
...
Ad_Object* Evaluator::EvalMinusPrefixOperatorExpression(Ad_Object* right) const {
    if (((Ad_Integer_Object*)right)->type != OBJ_INT) {
        return nullptr;
    }
    int value = ((Ad_Integer_Object*)right)->value;
    auto* obj = new Ad_Integer_Object(-value);
    garbageCollector->addObject(obj);
    return obj;
}

Ad_Object* Evaluator::EvalIdentifier(Ad_AST_Node* node, Environment &env) const {
    Ad_Object* obj;
    obj = nullptr;
...
Ad_Object* Evaluator::EvalBlockStatement(Ad_AST_Node* node, Environment &env) {
    ...
    for (auto* it: ((Ad_AST_BlockStatement*)node)->statements) {
        Ad_AST_Node *node = it;
        result = nullptr;
...
Ad_Object* Evaluator::ApplyFunction(Ad_Object* func, const std::vector<Ad_Object*>& args, Environment &env) {
    if (func->type == OBJ_FUNCTION) {
        auto* func_obj = (Ad_Function_Object*) func;
        if (func_obj->params.size() != args.size()) {
            // TODO: mark and sweep cleanup
            //for (int i = 0; i < args.size(); i++) free_Ad_Object_memory(args[i]);
            auto *obj = new Ad_Error_Object("function signature unrecognized, different number of params");
            garbageCollector->addObject(obj);
            return obj;
        }
    ...
    if (func->type == OBJ_BUILTIN) {
        auto* builtinObject = (Ad_Builtin_Object*) func;
        if (!builtinObject->acceptedNumbersOfArguments.empty() &&
                !validateNumberOfArguments(builtinObject->acceptedNumbersOfArguments, args.size())) {
            auto *obj = new Ad_Error_Object("builtin signature unrecognized, different number of params");
            garbageCollector->addObject(obj);
            return obj;
        }
    ...
    if (func->type == OBJ_CLASS) {
        auto* instance_environment = new Environment();
        instance_environment->ref_count = 1;
        garbageCollector->addEnvironment(instance_environment);
        auto* klass_object = (Ad_Class_Object*) func;
        auto* klass_ident = (Ad_AST_Identifier*) klass_object->name;
        auto* klass_instance = new Ad_Class_Instance(klass_ident->value, klass_object, instance_environment);
        ...
        for (auto* it: attributes) {
            ...
            if (node->type == ST_ASSIGN_STATEMENT) {
                auto* assign_statement = (Ad_AST_AssignStatement*) node;
                ...
                auto* assign_ident = (Ad_AST_Identifier*) assign_statement->name;
            ...
            if (node->type == ST_EXPRESSION_STATEMENT) {
                auto * expression_statement = (Ad_AST_ExpressionStatement*) node;
                if (expression_statement->expression->type == ST_ASSIGN_STATEMENT) {
                    auto* assign_statement = (Ad_AST_AssignStatement*) expression_statement->expression;
                    klass_instance->instance_environment->outer = &env;
                    Ad_Object* evaluated = Eval(assign_statement->value, *klass_instance->instance_environment);
                    auto* assign_ident = (Ad_AST_Identifier*) assign_statement->name;
                    std::string key = assign_ident->value;
        ...
        for (auto* it: methods) {
            auto* def_stmt = (Ad_AST_Def_Statement*) it;
            auto* method_obj = new Ad_Function_Object(def_stmt->parameters, def_stmt->body, klass_instance->instance_environment);
            garbageCollector->addObject(method_obj);
            auto* def_ident = (Ad_AST_Identifier*) def_stmt->name;
...
Ad_Object* Evaluator::ApplyMethod(Ad_Object* func, const std::vector<Ad_Object*>& args, Environment &env) {
    if (func->type == OBJ_FUNCTION) {
        auto* func_obj = (Ad_Function_Object*) func;
        if (func_obj->params.size() != args.size()) {
            auto *obj = new Ad_Error_Object("some error message here");
            garbageCollector->addObject(obj);
            return obj;
        }
...
Ad_Object* Evaluator::UnwrapReturnValue(Ad_Object* obj, Environment *env) {
    if (obj == nullptr) return obj; // found when doing the Java implementation
...
Ad_Object* Evaluator::EvalWhileExpression(Ad_AST_Node* node, Environment &env) {
    ...
    while (IsTruthy(condition)) {
        ...
        if (result != nullptr && result->Type() == OBJ_SIGNAL) return result; // exit() builtin was used in order to trigger the stopping of the process
        if (result != nullptr && result->Type() == OBJ_RETURN_VALUE) return result; // if a return is encountered in the block statement then that's the object the eval block must return
        if (result != nullptr && result->Type() == OBJ_BREAK) {
            // TODO: mark and sweep cleanup
            //free_Ad_Object_memory(result);
            return nullptr;
        }
        if (result != nullptr && result->Type() == OBJ_CONTINUE) {
            // TODO: mark and sweep cleanup
            //free_Ad_Object_memory(result);
        }
...
Ad_Object* Evaluator::EvalListIndexExpression(Ad_Object* left, Ad_Object* index) {
    ...
    if (idx < 0 || idx >= max) return nullptr;
    // TODO: mark and sweep cleanup
    //free_Ad_Object_memory(left); // this should have ref_count > 0 if store in a context variable
    //free_Ad_Object_memory(index);
...
Ad_Object* Evaluator::EvalHashLiteral(Ad_AST_Node* node, Environment &env) {
    ...
    auto* hash = new Ad_Hash_Object(pairs);
...
Ad_Object* Evaluator::evalStringIndexExpression(Ad_Object* left, Ad_Object* index) const {
    auto* obj = (Ad_String_Object*) left;
    int idx = ((Ad_Integer_Object*)index)->value;
...
Ad_Object* Evaluator::EvalAssignStatement(Ad_AST_Node* node, Environment &env) {
    ...
    if (assign_statement->name->type == ST_INDEX_EXPRESSION) {
        ...
        if (((Ad_AST_MemberAccess*)(assign_statement->name))->owner->type == ST_THIS_EXPRESSION) {
            auto* member_access = (Ad_AST_MemberAccess*) assign_statement->name;
            ...
            auto *member_access = (Ad_AST_MemberAccess*) assign_statement->name;
            auto *superExpression = (Ad_AST_Super_Expression*) member_access->owner;
            Environment *parentKlassEnv = env.outer->getSibling(superExpression->target->TokenLiteral());
            auto *member = (Ad_AST_Identifier*) member_access->member;
            Ad_Object *obj = Eval(assign_statement->value, env);
            ...
            auto* member_access = (Ad_AST_MemberAccess*) assign_statement->name;
            auto* owner = (Ad_AST_Identifier*) member_access->owner;
        ...
        auto* identifier = (Ad_AST_Identifier*)assign_statement->name;
...
Ad_Object* Evaluator::EvalIndexExpressionAssign(Ad_AST_Node* node, Environment &env) {
    auto *assign_statement = (Ad_AST_AssignStatement*)node;
    auto *index_expression = (Ad_AST_IndexExpression*)assign_statement->name;
    ...
    if (obj->Type() == OBJ_LIST) {
        ...
        auto* list_obj = (Ad_List_Object*)obj;
    ...
    if (obj->Type() == OBJ_HASH) {
        ...
        auto* hash_obj = (Ad_Hash_Object*)obj;
        std::string hash = std::to_string(hash_string(index->Hash()));

        auto it = hash_obj->pairs.find(hash);
...
Ad_Object* Evaluator::EvalDefStatement(Ad_AST_Node* node, Environment& env) const {
    auto* def_statement = (Ad_AST_Def_Statement*) node;

    std::vector<Ad_AST_Node*> parameters = def_statement->parameters;
    Ad_AST_Node* body = def_statement->body;

    auto* ident = (Ad_AST_Identifier*) def_statement->name;
    auto* func = new Ad_Function_Object(parameters, body, &env);
    garbageCollector->addObject(func);
    env.Set(ident->value, func);
    return nullptr; // this is correct, i don't want to print the function memory address on its definition statement
}

Ad_Object* Evaluator::EvalClassStatement(Ad_AST_Node* node, Environment& env) const {
    auto* class_node = (Ad_AST_Class*) node;
    auto* klass_object = new Ad_Class_Object(class_node->name, class_node->methods, class_node->attributes, class_node);
    garbageCollector->addObject(klass_object);
    klass_object->inheritFrom = class_node->inheritFrom;
    auto* klass_ident = (Ad_AST_Identifier*)(class_node->name);
...
void Evaluator::updateInstanceWithInheritedClasses(Ad_Object* obj, Environment& env) {
    auto* adClassInstance = (Ad_Class_Instance*) obj;
    std::vector<Ad_AST_Node*> superKlasses = ((Ad_Class_Object*)adClassInstance->klass_object)->inheritFrom;
    for (auto & superKlasse : superKlasses) {
        std::string identifier = superKlasse->TokenLiteral();
        ...
        for (auto* it: parentAdClassObject->attributes) {
            Ad_AST_Node *node = it;
            if (node->type == ST_ASSIGN_STATEMENT) {
        ...
        for (auto* it: parentAdClassObject->methods) {
            // this adds everything to main class
            auto* def_stmt = (Ad_AST_Def_Statement*) it;
            auto* method_obj = new Ad_Function_Object(def_stmt->parameters, def_stmt->body, adClassInstance->instance_environment);
            garbageCollector->addObject(method_obj);
            auto* def_ident = (Ad_AST_Identifier*) def_stmt->name;
...
Ad_Object* Evaluator::EvalMemberAccess(Ad_AST_Node* node, Environment& env) { // TODO: cleanup aici si in acelsi loc la implementarea in java
    ...
    if (member_access->owner->type == ST_THIS_EXPRESSION) {
        ...
        if (member_access->is_method) {
            ...
            if (klass_method == nullptr) {
                //return &NULLOBJECT;
                auto *obj = new Ad_Error_Object("method " + member->value + " not found in class " + ((Ad_Class_Object*) klass_instance->klass_object)->name->TokenLiteral());
                garbageCollector->addObject(obj);
                return obj;
            }
            std::vector<Ad_Object*> args_objs = EvalExpressions(member_access->arguments, env);
            ...
            if (member_access->owner->type == ST_IDENTIFIER) {
                ...
                //klass_environment->outer = &env;
                Environment* old = klass_environment->outer;
                klass_environment->outer = nullptr;
...
Ad_Object* Evaluator::evalRecursiveMemberAccessCall(Ad_AST_Node* node, Environment& env) {
    ...
    for (int i = chainedMemberAccesses.size() - 1; i >= 0; i--) {
        auto* currentMemberAccess = (Ad_AST_MemberAccess*) chainedMemberAccesses.at(i);
...
Ad_Object* Evaluator::recursiveMemberAccessAssign(Ad_AST_Node *node, Environment *env) {
    ...
    // TODO: add index expression as initial member access handler
    auto *assignStatement = (Ad_AST_AssignStatement*) node;
    auto *memberAccess = (Ad_AST_MemberAccess*) assignStatement->name;
    std::vector<Ad_AST_MemberAccess*> chainedMemberAccesses;
    ...
    auto *identifier = (Ad_AST_Identifier*) memberAccess->member;
    currentEnvironment->Set(identifier->value, obj);

    return nullptr;
}

Ad_Object* Evaluator::EvalFileObjectMethod(Ad_AST_Node* node, const std::vector<Ad_AST_Node*>& args, Environment& env) {
    // TODO: implement this
    auto* member_access = (Ad_AST_MemberAccess*) node;
    if (member_access->owner->type != ST_IDENTIFIER) {
        return nullptr;
    }
    auto* owner_ident = (Ad_AST_Identifier*) member_access->owner;
    Ad_Object* owner_obj_raw = env.Get(owner_ident->value);
    if (owner_obj_raw == nullptr) return nullptr;
    if (owner_obj_raw->type == OBJ_FILE) {
        auto* owner = (Ad_File_Object*) owner_obj_raw;
        ...
        if (method_name == "read") {
            if (owner->_operator == "r") {
                ...
                auto* result = new Ad_String_Object(data);
                garbageCollector->addObject(result);
                return result;
            }
        }
...
Ad_Object* Evaluator::evalSocketObjectMethod(Ad_AST_Node* node, std::vector<Ad_AST_Node*> args, Environment& env) {
    ...
    auto* owner_ident = (Ad_AST_Identifier*) member_access->owner;
    auto* member_ident = (Ad_AST_Identifier*) member_access->member;
    Ad_Object* owner_obj_raw = env.Get(owner_ident->value);
    ...
    if (owner_obj_raw->type == OBJ_THREAD) {
        if (member_access->is_method) {
            ...
            if (member_ident->value == "await") {
                ...
                auto* thread_object = (Ad_Thread_Object*) owner_obj_raw;
...
Ad_Object* Evaluator::EvalPrefixIncrement(Ad_AST_Node* node, Environment& env) const {
    auto* prefix_increment = (Ad_AST_PrefixIncrement*) node;
    auto* ident = (Ad_AST_Identifier*) prefix_increment->name;
    Ad_Object* old_obj = env.Get(ident->value);
    if (old_obj->Type() == OBJ_INT) {
        int value = ((Ad_Integer_Object*) old_obj)->value;
        auto* new_obj = new Ad_Integer_Object(value + 1);
        garbageCollector->addObject(new_obj);
        env.Set(ident->value, new_obj);
        auto* result = new Ad_Integer_Object(value + 1);
        garbageCollector->addObject(result);
        return result;
    }
    return &NULLOBJECT;
...
Ad_Object* Evaluator::EvalPostfixIncrement(Ad_AST_Node* node, Environment& env) {
    auto *expr = (Ad_AST_PostfixIncrement*) node;
    if (expr->name->type == ST_INDEX_EXPRESSION) {
        auto *indexExpression = (Ad_AST_IndexExpression*) (expr->name);
    ...
    auto* ident = (Ad_AST_Identifier*) expr->name;
    Ad_Object* old_obj = env.Get(ident->value);
    if (old_obj->Type() == OBJ_INT) {
        int value = ((Ad_Integer_Object*) old_obj)->value;
        if ("++" == expr->_operator) {
            auto *result = new Ad_Integer_Object(value);
            garbageCollector->addObject(result);
...
Ad_Object* Evaluator::EvalForExpression(Ad_AST_Node* node, Environment& env) {
    auto* expr = (Ad_AST_ForExprssion*) node;
    ...
    while (IsTruthy(condition)) {
        ...
        if (result != nullptr && result->Type() == OBJ_SIGNAL) return result;
        if (result != nullptr && result->Type() == OBJ_RETURN_VALUE) return result;
        if (result != nullptr && result->Type() == OBJ_BREAK) {
            // TODO: mark and sweep cleanup
            //free_Ad_Object_memory(result);
            return nullptr;
...
Ad_Object* Evaluator::EvalBreakStatement(Ad_AST_Node* node, Environment& env) const {
    auto *result = new Ad_Break_Object();
    garbageCollector->addObject(result);
    return result;
}

Ad_Object* Evaluator::EvalContinueStatement(Ad_AST_Node* node, Environment& env) const {
    auto *result = new Ad_Continue_Object();
    garbageCollector->addObject(result);
    return result;
}
...
Ad_Object* Evaluator::evalCallExpression(Ad_AST_Node* node, Environment *env) {
    //std::cout << "#";
    auto *callExpression = (Ad_AST_CallExpression*) node;
    Ad_Object* func = Eval(callExpression->function, *env);
    if (IsError(func)) return func;
    if (func->type == OBJ_NULL) {
        // function was not found
        auto *result = new Ad_Error_Object("function " + ((Ad_AST_Identifier*)callExpression->function)->value + " not found.");
        garbageCollector->addObject(result);
        return result;
    }
...
Ad_Object* Evaluator::EvalFloatExpression(Ad_AST_Node* node, Environment& env) const {
    auto* obj = new Ad_Float_Object();
    garbageCollector->addObject(obj);
...
Ad_Object* Evaluator::EvalString(Ad_AST_Node* node, Environment &env) const {
    auto* obj = new Ad_String_Object(((Ad_AST_String*)node)->value);
    garbageCollector->addObject(obj);
    return obj;
}

Ad_Object* Evaluator::NewError(std::string message) const {
    auto* obj = new Ad_Error_Object(message);
    garbageCollector->addObject(obj);
    return obj;
}

bool Evaluator::validateNumberOfArguments(std::vector<int> acceptedNumberArguments, int argumentSize) {
    //for (std::vector<int>::iterator it = acceptedNumberArguments.begin() ; it != acceptedNumberArguments.end(); ++it) {
    for (auto it: acceptedNumberArguments) {
