...
for test_file in test_files:
    ...
    if os_platform.lower() == "darwin":
        proc = subprocess.Popen(["./memory-check-macos.sh", "{0}{1}".format(binary_path, binary_excutable), "{0}{1}".format(tests_path, target)])
    elif os_platform.lower() == "linux":
        proc = subprocess.Popen(["./memory-check-linux.sh", "{0}{1}".format(binary_path, binary_excutable), "{0}{1}".format(tests_path, target)])
    else:
